...
class ReceiverMessageTests(unittest.TestCase):
    ...
    def test_send_bot_message_group_success_with_token(self):
        ...
        with mock.patch.object(self.receiver.urllib.request, "urlopen", side_effect=fake_urlopen):
    ...
    def test_send_bot_message_private_retries_then_success(self):
        ...
        with mock.patch.object(self.receiver.urllib.request, "urlopen", side_effect=flaky), mock.patch.object(
            self.receiver.time, "sleep", lambda *_: None
        ):
            ok, detail = self.receiver.send_bot_message("hello")
    ...
    def test_push_text_with_optional_image_modes(self):
        with mock.patch.object(self.receiver, "send_bot_message", return_value=(True, "ok")) as send_mock, mock.patch.object(
            self.receiver, "image_to_segment", return_value={"type": "image", "data": {"file": "base64://x"}}
        ):
            self.receiver.BOT_MESSAGE_MODE = "text"
            self.assertEqual(self.receiver.push_text_with_optional_image("txt", "/tmp/a.png"), (True, "ok"))
    ...
    def test_push_text_with_optional_image_fallback_to_text(self):
        self.receiver.BOT_MESSAGE_MODE = "text+image"
        with mock.patch.object(self.receiver, "image_to_segment", return_value=None), mock.patch.object(
...
class ReceiverFormattingAndCacheTests(unittest.TestCase):
                ...
                "userMysekaiHarvestMaps": [
                    {
                        "mysekaiSiteId": 7,
                        "userMysekaiSiteHarvestResourceDrops": [
                            {"resourceType": "mysekai_material", "resourceId": 12, "quantity": 1, "seq": 1},
                            {"resourceType": "mysekai_material", "resourceId": 12, "quantity": 3},
                            {"resourceType": "mysekai_material", "resourceId": 99, "quantity": 4},
                            {"resourceType": "mysekai_item", "resourceId": 7, "quantity": 2},
                        ],
                    },
                    {
                        "mysekaiSiteId": 8,
                        "userMysekaiSiteHarvestResourceDrops": [
                            {"resourceType": "mysekai_material", "resourceId": 12, "quantity": 2, "positionX": 8, "positionZ": 9}
                        ],
                    },
                ]
            }
    ...
    def test_cleanup_window_dedup_cache(self):
        ...
        with mock.patch.object(self.receiver.time, "time", return_value=now_ts), mock.patch.object(
            self.receiver, "save_dedup_cache"
        ) as save_mock:
...
class ReceiverIntegrationTests(unittest.TestCase):
    ...
    def test_process_notification_skips_non_full_packet(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with mock.patch.object(self.receiver, "append_notification_event") as append_mock:
                self.receiver.process_mysekai_notification(path, "https://x/user/1/mysekai")
                    ...
                    "userMysekaiHarvestMaps": [
                        {
                            "mysekaiSiteId": 6,
                            "userMysekaiSiteHarvestResourceDrops": [
                                {"resourceType": "mysekai_material", "resourceId": 11, "quantity": 1}
                            ],
                        }
                    ]
                }
    ...
    def test_process_notification_hit_flow(self):
        with tempfile.TemporaryDirectory() as td:
                    ...
                    "userMysekaiHarvestMaps": [
                        {
                            "mysekaiSiteId": 6,
                            "userMysekaiSiteHarvestResourceDrops": [
                                {"resourceType": "mysekai_material", "resourceId": 12, "quantity": 2, "seq": 1}
                            ],
                        }
                    ]
                }
            ...
            with mock.patch.object(self.receiver, "cleanup_window_dedup_cache"), mock.patch.object(
                self.receiver, "filter_hits_for_current_window", return_value=("20260320_0500", filtered_hits)
            ), mock.patch.object(self.receiver, "append_notification_event") as append_mock, mock.patch.object(
                self.receiver, "ensure_notification_dirs", return_value=(notification_dir, hit_dir)
            ), mock.patch.object(
                self.receiver, "render_mysekai_site_maps", return_value=({6: os.path.join(td, "m.png")}, "ok")
            ) as render_mock, mock.patch.object(
                self.receiver, "push_text_with_optional_image", return_value=(True, "ok")
            ) as push_mock, mock.patch.object(self.receiver, "prune_old_files"):
                self.receiver.process_mysekai_notification(out_json, "https://x/api/user/999/mysekai")
...
class RequestHandlerTests(unittest.TestCase):
    ...
    def _start_server(self):
        ...
        server = self.receiver.socketserver.TCPServer(("127.0.0.1", 0), self.receiver.RequestHandler)
